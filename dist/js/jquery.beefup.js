(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    ...
    factory();
    global.jqueryBeefup = mod.exports;
  }
  ...
  (function ($) {
    ...
    beefup.methods = {
       ...
       * @returns {*}
       */
      _getResponsiveVars: function _getResponsiveVars(vars) {
        var windowWidth = window.innerWidth || $(window).width(); // Sort
       ...
       */
      _animation: function _animation(type, $el, speed, callback) {
        switch (type) {
          case 'slideDown':
       ...
       * @return {undefined}
       */
      _close: function _close($el, vars) {
       ...
       * @returns {*}
       */
      _getStayOpen: function _getStayOpen($obj, value) {
        var $el;
       ...
       * @return {undefined}
       */
      _openSingle: function _openSingle($obj, $el, vars) {
        if (!vars.openSingle) {
          return;
        }
       ...
       * @param {object} vars
       */
      _addSelfCloseEvent: function _addSelfCloseEvent($obj, vars) {
        if (!vars.selfClose) {
          return;
        }
        ...
        $(document).on('click', function (e) {
          ...
          var $el = $obj.filter('.' + vars.openClass); // Exclude stayOpen item
       ...
       * @param {object} vars
       */
      _addHashchangeEvent: function _addHashchangeEvent($obj, vars) {
        if (!vars.hash) {
          return;
        }
       ...
       * @return {object}
       */
      _initAccessibility: function _initAccessibility($el, vars) {
        var $trigger = $el.find(vars.trigger + ':first'),
            id = 'acc' + beefup.id++,
            label = id + 'id',
        ...
        if (vars.accessibility) {
          ...
          $button.attr('aria-controls', id).attr('aria-expanded', false).attr('id', label);
          $el.find(vars.content + ':first').attr('aria-labelledby', label).attr('id', id).attr('role', 'region');
        }
    ...
    $.fn.beefup = function (options) {
      ...
      this.open = function ($el, callback) {
        ...
        $el.each(function () {
          var $this = $(this),
              ...
              animation = vars.animation === 'slide' ? 'slideDown' : vars.animation === 'fade' ? 'fadeIn' : 'show'; // Open single


          beefup.methods._openSingle($obj, $el, vars); // Fix #11: Remove hidden attribute before animation
          ...
          beefup.methods._animation(animation, $content, vars.openSpeed, function () {
            beefup.methods._open($this, vars); // Scroll
      ...
      this.close = function ($el, callback) {
        ...
        $el.each(function () {
          var $this = $(this),
              vars = beefup.methods._getVars($this),
              $content = $this.find(vars.content + ':first'),
              animation = vars.animation === 'slide' ? 'slideUp' : vars.animation === 'fade' ? 'fadeOut' : 'hide'; // Animation


          beefup.methods._animation(animation, $content, vars.closeSpeed, function () {
            beefup.methods._close($this, vars); // Callbacks
      ...
      this.scroll = function ($el) {
        ...
        }, vars.scrollSpeed).promise().done(function () {
      ...
      return this.each(function (index, el) {
        var $el = $(el),
            vars = beefup.methods._initAccessibility($el, $.extend({}, beefup.defaults, options, $el.data('beefup-options')));

        if ($el.data('beefup')) {
          return;
        }

        $el.data('beefup', vars); // Breakpoints
        ...
        beefup.methods._close($el.not('.' + vars.openClass), vars); // Callback
